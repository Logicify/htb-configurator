    ...
    public HTBClass read() {
        ...
        try{
            ...
            if(htb.isRoot())
            {
                ...
                if(htb.checkIfTrue("DEFAULT")) htb.getRootParams().setDefaultID(Integer.parseInt(values.get("DEFAULT")));
                if(htb.checkIfTrue("R2Q")) htb.getRootParams().setR2q(Integer.parseInt(values.get("R2Q")));
                if(htb.checkIfTrue("DCACHE")) htb.getRootParams().setDchache(values.get("DCACHE").equals("yes") ? true : false);
            }
            ...
            if(htb.checkIfTrue("LEAF")){
                ...
                switch(lf)
                {
                    case "sfq": htb.setLeaf(Leaf.SFQ); break;
                    case "pfifo": htb.setLeaf(Leaf.PFIFO); break;
                    case "bfifo": htb.setLeaf(Leaf.BFIFO); break;
                    default: htb.setLeaf(Leaf.NONE); break;
                }
            }
